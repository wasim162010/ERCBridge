...
contract ERC20TokenBridge {
  ...
  //If the bridge operators make a mistake then auditors add a penalty.
  //We have nodes, audit nodes verifier nodes and operator nodes with manual over ride for bigger sums
  uint public penalty_amount;
  ...
  function release(
    ...
    string calldata txHash
        ...
        erc20.transfer(request.to, request.amount - fee_amount);

        if(penalty_amount > 0) {
        if(penalty_amount > fee_amount) {
            erc20.mint(penalty_wallet, fee_amount);
            penalty_amount -= fee_amount;
        }
        else {
            erc20.mint(penalty_wallet, penalty_amount);
            erc20.mint(msg.sender, fee_amount - penalty_amount);
            penalty_amount -= penalty_amount;
        }
        
        }
        else {
        erc20.mint(msg.sender, fee_amount);
        }
        operating_limits[msg.sender] -= amount;
        emit Release(request_id, request.to, request.amount - fee_amount);
  }

  function initERC20(address _addr) external {
        erc20 = IERC20(_addr);
  }
